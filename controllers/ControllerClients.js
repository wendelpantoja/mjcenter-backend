...
class Clients {
    async getClients(req, res) {
        try {
            ...
            const cpf = req.body.doc.replace(/[.-]/g, '')

            const data = await instance.get(`/apps/api/terceiros/?inicio=0&quantidade=10&tipopessoa=pf&nome=${nameUrl}&documento=${cpf}&token=${process.env.API_AUTHORIZATION_CODE}`)
